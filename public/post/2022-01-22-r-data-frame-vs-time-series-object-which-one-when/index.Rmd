...
*(Let me say right at the start that although time series tools are powerful for forecasting, that is not the main focus here and I'll only touch on that briefly below.)*
...
**xts** is a leading, evolved R package for working with time series. It builds on **zoo**, an earlier pkg for handling time series in R. **Datacamp** has an excellent [xts Cheat Sheet](http://datacamp-community-prod.s3.amazonaws.com/72771032-0653-4d87-a798-4a83879e99c8) and very helpful [supplementary info.](https://www.datacamp.com/community/blog/r-xts-cheat-sheet)  
...
* monthly Air Passengers - yes, I know everybody uses Air Passengers for their time series example. So damn handy. ;)
...
There has to be enough information to assess any seasonal trend, so this requires:
...
```{r}
## if we try with Nile we get error, since annual data and can't detect seasonality
#nile_decomp <- decompose(Nile)
## Air Passengers works
...
Here is a quick example to show how easy forecasting can be in R. Note that we need to bring in the **forecast** package for this.
...
Hopefully this helps you make the most of your 'time'!
